...
const generarExcel = async (req, res = response) => {
  ...
  const imagenesEstampillasBD = await ImagenesEstampillas.aggregate(
    [
...

     {
       $project: {
        catalogo:1,
         arrEstampilla: { $arrayElemAt: ["$arrEstampilla", 0] },
         codigo_estampilla:1
       }
     },
     {
  ...
  // //Buscando imagenes no asociadas a estampillas
  // if (imagenesEstampillasBD != null) {
  //   var codigosEs = [];

  //   imagenesEstampillasBD.map(data =>
  //     {
  //       codigosEs.push(data.codigo_estampilla);
  //     });
    
  //   var imagenesNoAsociadas = await ImagenesEstampillas.find( { CODIGO: codigosEs } );
  //   console.log("imagenesNoAsociadas ->", imagenesNoAsociadas);
  ...
  //   return res.json({
  //     ok:false,
  //     msg: "No es posible generar el excel, debes agregar imagenes antes de generar el documento."
