...
async function crearDatosEnvio(objeto) {
    ...
    try {
        ...
        var verificarEditar = await Datos_envio.findOneAndUpdate({ usuario: objeto.usuario}, objeto  );
...
async function mostrarDatosEnvioIdUsuario  (usuario) {
  ...
  try {
    ...
    if (!productoBD) {
      ...
      objetoRespuesta.msg = "El id no cuenta con datos de envio asociados al carrito.";
    ...
    console.log(
      "Error en catch de mostrarDatosEnvioIdUsuario | middelwares ventas ", error
    );
    objetoRespuesta.ok = false;
    objetoRespuesta.msg = "" + error;
    objetoRespuesta.tipo_error = "Catch mostrarDatosEnvioIdUsuario. | middelwares ventas";
      ...
      [
        {
          ...
          cod: 1,
          descripcion: "El vendor ha verificado el pago y ha concluido que el pago es procedente."
        },
        {
          nombre_estado: "Pago Aceptado - Por Enviar",
          cod: 2,
          descripcion: "El pago ha sido verificado correctamente, el producto está en preparación para envío."
        },
        {
          nombre_estado: "Pago Aceptado - Producto Enviado",
          cod: 3,
          descripcion: "El pago ha sido verificado correctamente, el producto va en camino."
        },
        {
          nombre_estado: "Exitosa - Producto Entregado",
          cod: 4,
          descripcion: "La venta se ha concretado correctamente, el producto ya se encuentra en manos del comprador."
        }

      ]);

  console.log("   | Se han creado "+estadosBD.length+" estados venta en BD.");
